...
int main(int argc, char ** argv)
{
    ...
    for (int x=0; x<5; x++) {
      ...
      for (int y=0; y<3; y++) {
        printf("alpha = %f", shells[x].alpha[y]);
        printf("   coef = %f \n", shells[x].coef[y]);
      }
    }
    printf("\n\n\n");
    ...
    for (int x=0; x<5; x++) {
      //std::cout << "AM = " << shells[x].am << std::endl;
      //std::cout << "X = " << shells[x].x << std::endl;
      for (int y=0; y<3; y++) {
        printf("alpha = %f", shells[x].alpha[y]);
        printf("   coef = %f \n", shells[x].coef[y]);
    ...
    double * work = (double *)SIMINT_ALLOC(simint_ostei_workmem(0, 1));
    ...
    for(int j = 0; j <= i; j++)
    {
        ...
        printf("am1 = %d    am2 = %d   nshell = %d\n",left_pair.am1,left_pair.am2,left_pair.nshell12);
