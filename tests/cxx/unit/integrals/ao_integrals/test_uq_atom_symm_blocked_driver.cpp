...
auto corr_answer(const simde::type::tensor& T) {
    if constexpr(std::is_same_v<UQType, double>) {
        ...
        corr_buffer.set_elem(
          {0, 0, 0, 0}, types::construct_uq_type<UQType>(0.774606, value_t(0)));
        corr_buffer.set_elem({0, 0, 0, 1}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem({0, 0, 1, 0}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem(
          {0, 0, 1, 1}, types::construct_uq_type<UQType>(0.446701, value_t(0)));
        corr_buffer.set_elem({0, 1, 0, 0}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem({0, 1, 0, 1}, types::construct_uq_type<UQType>(
                                             0.120666, 0.0000170000000000));
        corr_buffer.set_elem({0, 1, 1, 0}, types::construct_uq_type<UQType>(
                                             0.120666, 0.0000170000000000));
        corr_buffer.set_elem({0, 1, 1, 1}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem({1, 0, 0, 0}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem({1, 0, 0, 1}, types::construct_uq_type<UQType>(
                                             0.120666, 0.0000170000000000));
        corr_buffer.set_elem({1, 0, 1, 0}, types::construct_uq_type<UQType>(
                                             0.120666, 0.0000170000000000));
        corr_buffer.set_elem({1, 0, 1, 1}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem(
          {1, 1, 0, 0}, types::construct_uq_type<UQType>(0.446701, value_t(0)));
        corr_buffer.set_elem({1, 1, 0, 1}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem({1, 1, 1, 0}, types::construct_uq_type<UQType>(
                                             0.265558, 0.0000010000000000));
        corr_buffer.set_elem(
          {1, 1, 1, 1}, types::construct_uq_type<UQType>(0.774606, value_t(0)));
        return T_corr;
    }
}
    ...
    SECTION("H2") {
        ...
        std::string uq_type = []() -> std::string {
            if constexpr(tensorwrapper::types::is_uncertain_v<float_type>) {
                return "uncertain";
            } else if constexpr(tensorwrapper::types::is_interval_v<
                                  float_type>) {
                return "interval";
