...
TEST_CASE("Cauchy-Schwarz") {
    ...
    auto [cs_mat_eri] = mm.at("Shell Norms Coulomb").run_as<cs_approx>(bs, bs, deriv);
    auto [cs_mat_stg] = mm.at("Shell Norms STG").run_as<cs_approx>(bs, bs, deriv);
    auto [cs_mat_yuk] = mm.at("Shell Norms Yukawa").run_as<cs_approx>(bs, bs, deriv);
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 5; ++j) {
