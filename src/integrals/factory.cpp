...
integrals::Factory::Factory(integrals::property prop, integrals::NWX_basis bs1, integrals::NWX_basis bs2, integrals::NWX_basis bs3, integrals::NWX_basis bs4) {
            if (instances==0 && not libint2::initialized()) {
                libint2::initialize();
            };
            instances++;
            libint2::Operator op;
            switch (prop) {
                ...
                case integrals::property::eri:
                    ...
                    break;
                default:
                    assert(false && "invalid integral case in switch");
...
integrals::Factory::Factory(integrals::property prop, integrals::NWX_basis bs1, integrals::NWX_basis bs2, integrals::NWX_basis bs3) : 
            Factory(prop, bs1, bs2, bs3, bs3) {};

integrals::Factory::Factory(integrals::property prop, integrals::NWX_basis bs1, integrals::NWX_basis bs2) : 
            Factory(prop, bs1, bs2, bs2) {};

integrals::Factory::~Factory() {
            // delete engine; Apparently std::unique_ptr self destructs.
            instances--;
            if (instances==0 && libint2::initialized()) {
                libint2::finalize();
            };
        };
        ...
        const double* const integrals::Factory::compute(size_t s1, size_t s2, size_t s3, size_t s4) {
