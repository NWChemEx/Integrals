...
inline auto shells2ord(const std::vector<libint2::BasisSet>& bases,
    ...
    size_vector_t lo_ao(N);
    size_vector_t up_ao(N);
    for(auto i = 0; i < N; ++i) {
        lo_ao[i] = bases[i].shell2bf()[shells[i]];
        up_ao[i] = lo_ao[i] + bases[i][shells[i]].size();
    }

    /// Calculate the ordinal step of each basis dimension other than the Nth.
    size_vector_t step_size;
    for(auto i = 0; i < N - 1; ++i) {
        auto step = bases[N - 1].nbf();
        for(auto j = i + 1; j < N - 1; ++j) step *= bases[j].nbf();
    ...
    while(curr_ao[0] = up_ao[0]) {
        /// ordinal calculation
        auto curr_ord = curr_ao[N - 1];
        for(auto i = 0; i < N - 1; ++i) {
            curr_ord += curr_ao[i] * step_size[i];
        }
