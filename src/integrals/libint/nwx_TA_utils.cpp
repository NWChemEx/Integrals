...
template<typename T>
TA::TiledRange1 _make_tiled_range(basis<T> basis_set,
    ...
    for(const auto& atom : basis_set) {
        ...
        sizes_index++;
        if(sizes_index >= tile_sizes.size()) { sizes_index = 0; }
    }
...
TA::TiledRange1 make_tiled_range(basis<double> basis_set,
                                 std::vector<size> tile_sizes) {
    return _make_tiled_range<double>(std::move(basis_set),
                                     std::move(tile_sizes));
}

TA::TiledRange1 make_tiled_range(basis<float> basis_set,
                                 std::vector<size> tile_sizes) {
    return _make_tiled_range<float>(std::move(basis_set),
                                    std::move(tile_sizes));
}
...
template<typename basis_type>
TA::TiledRange _make_trange(
  const std::vector<basis_type>& basis_sets,
  ...
  std::vector<TA::TiledRange1> ranges) {
    for(const auto& basis_set : basis_sets) {
        ...
        for(const auto& atom_range : atom_ranges) {
            ...
            // Add the total basis set count for current atom range to the last
            // bound
            for(auto atom = atom_range.first; atom < atom_range.second;
                ++atom) {
...
TA::TiledRange select_tiling(
  ...
  const std::vector<std::pair<size, size>>& atom_ranges,
  std::vector<TA::TiledRange1> ranges) {
    return _select_tiling(basis_sets, tile_sizes, atom_ranges,
                          std::move(ranges));
}
